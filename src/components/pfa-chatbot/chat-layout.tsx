...
export function ChatLayout({ userId }: ChatLayoutProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const result = await handleUserMessage(input, userId);
      if (result.response) {
  ...
  return (
    <Card className="w-full max-w-3xl mx-auto h-[calc(100vh-10rem)] flex flex-col">
      <CardHeader>
        ...
        <p className="text-sm text-muted-foreground">This is a safe space to talk. Your conversation is private.</p>
      </CardHeader>
